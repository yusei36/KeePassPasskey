...
The thick arrow is what makes your passkeys show up in Windows sign-in dialogs at all: as you open or save your database, the passkey metadata (site and user name, never the keys themselves) is written to the Windows credential cache. That is why the database has to be unlocked, and why [no passkeys on this device](docs/troubleshooting-faq.md#the-website-says-there-are-no-passkeys-on-this-device) is usually a locked database.
